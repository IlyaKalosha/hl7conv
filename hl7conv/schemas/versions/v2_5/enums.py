...
class NatureofService_Test_Observation(str, Enum):
    ...
    routines = CBC, UA, electrolytes) C  Single observation calculated via a rule or formula from other independent
...
class CompletionStatusforvalidvalues(str, Enum):
    """
    322 - Completion Status for valid values.
...
class Alternatecharactersethandlingscheme(str, Enum):
    ...
    Technique". . 2.3  The character set switching mode specified in HL7 2.5, section 2.7.2, "Escape sequences
    supporting multiple character sets" and section 2.A.46, "XPN - extended person name". <null>  This is the
...
class DenialorRejectionCode(str, Enum):
    ...
    2  Line item is on a multiple-day claim. The line item is not denied or rejected, but occurs on a
...
class SignatureCode(str, Enum):
    ...
    C  Signed CMS-1500 claim form on file, e.g. authorization for release of any medical or
    other information necessary to process this claim and assignment of benefits.
