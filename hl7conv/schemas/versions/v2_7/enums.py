...
class ObservationResultStatusCodesInterpretation(str, Enum):
    ...
    observation identified in the OBX was not sought when the universal
...
class NatureofService_Test_Observation(str, Enum):
    ...
    (e.g., routines = CBC, UA, electrolytes)This set indicates that the code being described is used to order multiple
    service/test/observation b
    C  Single observation calculated via a rule or formula from other independent observations
...
class ResponseLevel(str, Enum):
    ...
    ER  Error/Reject conditions only.  Only MFA segments denoting errors must be returned via the
    ...
    SU  Success.  Only MFA segments denoting success must be returned via the application-level
...
class AddressType(str, Enum):
    ...
    O  Office/Business
    P  Permanent
    RH  Registry home. Refers to the information system, typically managed by a public health agency,
     that stores patient information such as immunization histories or cancer data, regardless of where
...
class DurationCategories(str, Enum):
    ...
    PT  To identify measures at a point in time.  This is a synonym for "spot" or "random"
    as applied to urine measurements.
...
class AppointmentTypeCodes(str, Enum):
    ...
    Complete  A request to add a completed appointment, used to maintain records of completed appointments that
    did not appear in the schedule (e.g., STAT, walk-in, etc.)
    """
...
class AllowSubstitutionCodes(str, Enum):
    ...
    Notify  Notify the Placer Contact Person, through normal institutional procedures, that a substitution
...
class CPRangeType(str, Enum):
    ...
    P  Pro-rate. Apply this price to this interval, pro-rated by whatever portion of the
    interval has occurred/been consumed
    F  Flat-rate. Apply the entire price to this interval, do not pro-rate the price
    if the full interval has not occurred/been consumed
    """
...
class Encoding(str, Enum):
    ...
    Base64  Encoding as defined by MIME (Multipurpose Internet Mail Extensions) standard RFC 1521.
    Four consecutive ASCII characters represent three consecutive
    octets of binary data. Base64 utilizes a 65-character subset of US-ASCII, consisting of both the upper and
    """
...
class UniversalIDType(str, Enum):
    ...
    CLIA  Clinical Laboratory Improvement Amendments. Allows for the ability to designate organization identifier
    as a "CLIA" assigned number (for labs)
    CLIP  Clinical laboratory Improvement Program. Allows for the ability to designate organization identifier
    as a "CLIP" assigned number (for labs).Â  Used by US Department of Defense.
    GUID  Same as UUID.
    HCD  The CEN Healthcare Coding Scheme Designator
    HL7  HL7 registration schemes
    ISO  An International Standards Organization Object Identifier (OID), in accordance with ISO/IEC 8824.
     Formatted as decimal digits separated by periods; recommended limit of 64 characters
...
class CodingSystem(str, Enum):
    ...
    PLT  CDC PHLIP Lab test codes, where LOINC concept is too broad or not yet available, especially as
    needed for ordering and or lab to lab reporting )
    POS  POS Codes
...
class OCEEditCode(str, Enum):
    """
         ...
         10  Non-covered  service submitted for verification of denial (condition code 21 from header
         ...
         32  Partial hospitalization claim spans 3 or less days with in-sufficient services, or ECT or
...
class DenialorRejectionCode(str, Enum):
    ...
    2  Line item is on a multiple-day claim. The line item is not denied or rejected, but occurs on a day
    that has been denied or rejected.
    """
...
class ActPriority(str, Enum):
    ...
    CSR  Contact recipient for scheduling - Filler should contact the service recipient (target) to
    ...
    PRN  As needed - An "as needed" order should be accompanied by a description of what constitutes a need.
    This description is represented by an observation service predicate as a precondition.
    ...
    T  Timing critical - It is critical to come as close as possible to the requested time (e.g. for a
...
class Confidentiality(str, Enum):
    ...
    D  Clinician - Only clinicians may see this item, billing and administration persons can not access
     this item without special permission.
    I  Individual - Access only to individual persons who are mentioned explicitly as actors of this service and whose
    actor type warrants that access (cf. to actor typed code).
    L  Low - No patient record item can be of low confidentiality.  However, some service objects are not
    patient related and therefore may have low confidentiality.
    N  Normal - Normal confidentiality rules (according to good health care practice) apply, that is,
    only authorized individuals with a legitimate medical or business need may access this item.
    ...
    C  Celebrity - Celebrities are people of public interest (VIP) including employees,
    whose information require special protection.
    S  Sensitive - Information for which the patient seeks heightened confidentiality.
    Sensitive information is not to be shared with family members.
    Information reported by the patient about family members is sensitive by default.  Flag can be set or cleared
    T  Taboo - Information not to be disclosed or discussed with patient except through physician assigned to patient
    in this case.  This is usually a temporary constraint only; example use is a new fatal diagnosis or
    finding, such as malignancy or HIV.
    """
